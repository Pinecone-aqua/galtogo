    ...
    <div>
      <div className="mx-auto">
        <div className="flex justify-center my-10 md:flex">
          <h1 className="font-extrabold mx-[12px] text-center text-[32px] sm:text-[40px] md:text-[40px]">
      ...
      <div className="flex flex-col gap-8 md:flex md:flex-row">
        <div className="flex overflow-x-auto no-scrollbar gap-3 md:flex md:flex-col">
          {BranchItems.map((item, index) =>
            listId == index ? (
              <div
                key={index}
                className="flex gap-2 items-center px-[72px] py-[16px] bg-white shadow-[0_2px_20px_rgb(0,0,0,0.05)] rounded-lg cursor-pointer font-medium md:w-[320px]"
                onClick={() => {
              ...
              <div
                key={index}
                className="flex gap-2 items-center px-[16px] py-[20px] bg-white rounded-lg cursor-pointer md:w-[320px]"
                onClick={() => {
                  setListId(item.id), setCurrentLocation(item.location);
                }}
              >
        ...
        <div className="w-full h-[250px] lg:h-[420px] sm:h-[320px] md:rounded-lg overflow-auto">
          {!isLoaded ? (
            <h2>Loading...</h2>
          ) : (
            <GoogleMap
