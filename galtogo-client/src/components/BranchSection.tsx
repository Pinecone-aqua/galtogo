...
export default function BranchSection() {
  ...
  return (
    <div>
      {/* <Image src="https://www.cloudinary/image/brokoli"/> */}
      <div className="mx-auto flex flex-col justify-center w-[1364px]  py-20">
        <div className="flex justify-center text-[64px]">
          <h1 className="bg-gradient-to-r from-orange-400 to-orange-600 bg-clip-text text-transparent font-extrabold px-2">
            Yuna restaurant’s
          </h1>
        ...
        <div className="flex flex-col justify-center gap-3">
          {BranchItems.map((item, index) =>
            listId == index ? (
              <div
                key={index}
                className="flex gap-2 items-center bg-white shadow-[0_2px_20px_rgb(0,0,0,0.05)] p-5 w-[350px] rounded-lg cursor-pointer font-medium"
                onClick={() => {
              ...
              <div
                key={index}
                className="flex gap-2 items-center bg-white  p-5 w-[350px] rounded-lg cursor-pointer"
                onClick={() => {
                  setListId(item.id), setCurrentLocation(item.location);
                }}
              >
